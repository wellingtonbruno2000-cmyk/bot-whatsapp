...
async function handleMessage(phone, text) {
  ...
  if (parsed.intent === 'finance_add' || /^entrada |^saida /.test(lower)) {
    const type = parsed.type || (lower.startsWith('entrada') ? 'entrada' : 'saida');
    const amountMatch = lower.match(/(\d+[\d.,]*)/);
    const amount = parsed.amount || (amountMatch ? Number(amountMatch[1].replace(',', '.')) : 0);
    const description = parsed.description || lower.replace(/^entrada |^saida /, '').replace(/\d+[\d.,]*/, '').trim() || 'geral';
    const category = parsed.category || description;
    const entry = finance.addEntry(phone, { type, amount, description, category });
    const balance = finance.getOverallBalance(phone);
    return finance.formatEntryResponse(entry, balance);
  }

  if (parsed.intent === 'finance_summary' || lower === 'resumo') {
    return finance.formatSummaryResponse(finance.getSummary(phone));
  }

  if (parsed.intent === 'finance_balance' || lower === 'saldo') {
  ...
  if (parsed.intent === 'financing_add' || lower.startsWith('adicionar financiamento')) {
    ...
    const item = finance.addFinancing(phone, {
      ...
      monthly_amount: parsed.monthly_amount || (amountMatch ? Number(amountMatch[1].replace(',', '.')) : 0)
    });
  ...
  return 'Posso te ajudar com finanças, compromissos, lembretes e financiamentos. Exemplos: "saida 45 mercado", "resumo", "agendar visita amanhã 15:00", "adicionar financiamento carro 48x 1450".';
}
